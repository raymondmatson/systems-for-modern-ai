...
Version 1 remains SVG-first. Phase-6 benchmark version 3 now measures the **current Explore view-model plus the exact `src/app/explore` component tree** rather than using the former synthetic representative SVG. In the current H100 Representative baseline it materializes 348 SVG descendants and 16 SVG focus targets (356 descendants in the stressed pilot state), with zero Anatomy focus targets across all seven frozen Phase-5A pilot states. System-Chromium timings and the separate synthetic headroom series still support no Canvas/WebGL dense backplane and no renderer virtualization for the current initial-five scenes. The benchmark evidence is recorded under `reports/implementation/render-benchmark.json`; its benchmark JSX adapter bypasses React reconciliation and therefore does **not** replace the pinned Vite/Playwright browser matrix.
...
Phase 7A is **completed by project-owner user-confirmed external validation**. The historical protocol/readiness/template artifacts remain under `reports/testing/`; this Visual Design handoff did not run moderated participant sessions and does not infer participant metrics that were not supplied. Closure attribution is recorded in `reports/testing/phase7a-user-confirmed-validation.json`.

Phase 8 has completed its all-entered-context implementation/audit pass but remains **open pending a fresh pinned post-change regression/accessibility run**. The exhaustive current-renderer audit covers all 65 enterable initial-five contexts plus 65 deterministic stress variants and records zero audit errors/warnings or rendered node-label truncation contexts after bounded routing/density corrections. Because those corrections change shared `layout.ts` and `connectionVisuals.ts` after the last user-confirmed pinned run, Phase 8 cannot close by projecting earlier validation onto new code. See `docs/VISUAL_DESIGN_PHASE8_COVERAGE.md`, `reports/implementation/visual-coverage-initial-five.json`, and `reports/implementation/phase8-regression-revalidation.json`.

Playwright test locators for short generic actions must use exact accessible-name matching. In particular, use the shared `enterAction(page)` helper (or `getByRole('button', {name: 'Enter', exact: true})`); loose `Enter` matching can also match entity/relationship names containing strings such as `Data-center`.
